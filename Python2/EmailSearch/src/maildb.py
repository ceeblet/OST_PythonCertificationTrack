...
def msg_by_id(id):
    """
    Return the (presumably singleton) message whos primary key is given
...
def msgs(mindate=None, maxdate=None, namesearch=None, addsearch=None):
    ...
    If mindate is not specified, there is no lower bound on date, and similarly
...
#        raise TypeError("Must provide at least one of mindate, or maxdate")
    conds = []
    data = []
    if mindate:
